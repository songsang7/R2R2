...
- Actor takes [observation, z_state] with projection
- Critic takes [observation, z_state, action, z_next_state] with projection
- ObsProjector (1-layer MLP + ReLU) for projecting observations
"""
...
from scale_rl.agents.simbaV2_spl_r2r2.update import (
...
class SimbaV2SplR2R2Agent(BaseAgent):
    """
    ...
    def __init__(
        ...
        super(SimbaV2SplR2R2Agent, self).__init__(
            observation_space,
            action_space,
            cfg,
        )

        self._cfg = SimbaV2SplR2R2Config(**cfg)
    ...
    def update(self, update_step: int, batch: Dict[str, np.ndarray]) -> Dict:
        ...
        (
            ...
            coef_v=self._cfg.coef_var,
            coef_i=self._cfg.coef_spl,
            coef_c=self._cfg.coef_rr,
        )
